...
class trainNetwork:
    def __init__(self, agent, game, writer, buffer, BATCH, device):
        """
        Initialize the training process with the given agent, game, writer, buffer, batch size, and device
        """
        self.agent = agent
        ...
        self.eps = 1.0 # Initial epsilon for exploration

    def cache(self, state, action_idx, reward, next_state, terminal):
        """
        Store the experience to self.memory (replay buffer)
        """
        # Detach and clone tensors to move them to CPU and store in the buffer
